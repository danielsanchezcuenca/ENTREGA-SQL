...
CREATE TABLE flotaVehiculos.Grupo (
...
CREATE TABLE flotaVehiculos.Modelo (
    IDModelo SERIAL PRIMARY KEY,
    NombreModelo VARCHAR(255),
    IDMarca INT
);

CREATE TABLE flotaVehiculos.Seguro (
    IDSeguro SERIAL PRIMARY KEY,
    NombreCompañía VARCHAR(255),
    NúmeroPóliza VARCHAR(255)
);

CREATE TABLE flotaVehiculos.Coche (
    IDCoche SERIAL PRIMARY KEY,
    Color VARCHAR(255),
...
CREATE TABLE flotaVehiculos.Revisión (
    IDRevisión SERIAL PRIMARY KEY,
    KmsRevisión INT,
    FechaRevisión DATE,
...
INSERT INTO flotaVehiculos.Grupo(NombreGrupo) VALUES ('PSA Peugeot S.A.');
ALTER TABLE flotaVehiculos.Marca ADD CONSTRAINT FK_Marca_Grupo FOREIGN KEY (IDGrupo) REFERENCES flotaVehiculos.Grupo(IDGrupo);
ALTER TABLE flotaVehiculos.Modelo ADD CONSTRAINT FK_Modelo_Marca FOREIGN KEY (IDMarca) REFERENCES flotaVehiculos.Marca(IDMarca);
ALTER TABLE flotaVehiculos.Coche ADD CONSTRAINT FK_Coche_Modelo FOREIGN KEY (IDModelo) REFERENCES flotaVehiculos.Modelo(IDModelo);
ALTER TABLE flotaVehiculos.Coche ADD CONSTRAINT FK_Coche_Seguro FOREIGN KEY (IDSeguro) REFERENCES flotaVehiculos.Seguro(IDSeguro);
ALTER TABLE flotaVehiculos.Revisión ADD CONSTRAINT FK_Revisión_Coche FOREIGN KEY (IDCoche) REFERENCES flotaVehiculos.Coche(IDCoche);

CREATE TABLE flotavehiculos.coches (
...
INSERT INTO flotavehiculos.coches (matricula,grupo,marca,modelo,fecha_compra,color,aseguradora,n_poliza,fecha_alta_seguro,importe_revision,moneda,kms_revision,fecha_revision,kms_totales) VALUES
...
-- Insertar grupos únicos en la tabla Grupo
INSERT INTO flotaVehiculos.Grupo (NombreGrupo)
SELECT DISTINCT grupo
FROM flotavehiculos.coches
WHERE grupo IS NOT NULL;

-- Insertar las marcas únicas en la tabla Marca
INSERT INTO flotaVehiculos.Marca (NombreMarca, IDGrupo)
SELECT DISTINCT C.marca, G.IDGrupo
FROM flotavehiculos.coches C
JOIN flotaVehiculos.Grupo G ON C.grupo = G.NombreGrupo
WHERE C.marca IS NOT NULL;

-- Insertar modelos únicos en la tabla Modelo
INSERT INTO flotaVehiculos.Modelo (NombreModelo, IDMarca)
SELECT DISTINCT C.modelo, M.IDMarca
FROM flotavehiculos.coches C
JOIN flotaVehiculos.Marca M ON C.marca = M.NombreMarca
WHERE C.modelo IS NOT NULL;

-- Insertar los seguros únicos en la tabla Seguro
INSERT INTO flotaVehiculos.Seguro (NombreCompañía, NúmeroPóliza)
SELECT DISTINCT aseguradora, CAST(n_poliza AS VARCHAR)
FROM flotavehiculos.coches
WHERE aseguradora IS NOT NULL AND n_poliza IS NOT NULL;

-- Insertar los coches en la tabla Coche
INSERT INTO flotaVehiculos.Coche (Color, Matricula, TotalKilómetros, FechaCompra, IDModelo, IDSeguro)
SELECT color, matricula, kms_totales, fecha_compra, Mo.IDModelo, S.IDSeguro
FROM flotavehiculos.coches C
JOIN flotaVehiculos.Modelo Mo ON C.modelo = Mo.NombreModelo
JOIN flotaVehiculos.Seguro S ON C.aseguradora = S.NombreCompañía AND CAST(C.n_poliza AS VARCHAR) = S.NúmeroPóliza;

-- Insertar revisiones en la tabla Revisión
INSERT INTO flotaVehiculos.Revisión (KmsRevisión, FechaRevisión, Importe, Moneda, IDCoche)
SELECT kms_revision, fecha_revision, importe_revision, moneda, Co.IDCoche
FROM flotavehiculos.coches C
JOIN flotaVehiculos.Coche Co ON C.matricula = Co.Matricula
WHERE kms_revision IS NOT NULL AND fecha_revision IS NOT NULL AND importe_revision IS NOT NULL;

SELECT 
    M.NombreModelo AS "Nombre Modelo",
    Ma.NombreMarca AS "Marca",
    G.NombreGrupo AS "Grupo Empresarial",
    C.FechaCompra AS "Fecha de Compra",
    C.Matricula AS "Matrícula",
    C.Color AS "Color del Coche",
    C.TotalKilómetros AS "Total Kilómetros",
    S.NombreCompañía AS "Empresa Aseguradora",
    S.NúmeroPóliza AS "Número de Póliza"
FROM 
    flotaVehiculos.Coche C
JOIN flotaVehiculos.Modelo M ON C.IDModelo = M.IDModelo
JOIN flotaVehiculos.Marca Ma ON M.IDMarca = Ma.IDMarca
JOIN flotaVehiculos.Grupo G ON Ma.IDGrupo = G.IDGrupo
JOIN flotaVehiculos.Seguro S ON C.IDSeguro = S.IDSeguro
ORDER BY 
    C.FechaCompra DESC, M.NombreModelo ASC;
